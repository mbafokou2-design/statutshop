...
export const bot: TelegramBot | null = TOKEN
  ? new TelegramBot(TOKEN, { polling: !webhookUrl as any })
  : null;
...
if (bot) {
  ...
  console.log('[Telegram] Bot token not provided. Telegram bot is disabled.');
}

export async function sendTelegramOtp(phone: string, code: string): Promise<void> {
  if (!bot) {
    throw new Error(
      "Le bot Telegram n'est pas configuré sur ce serveur (TELEGRAM_BOT_TOKEN manquant)."
    );
  }
